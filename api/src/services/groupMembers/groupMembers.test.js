...
describe('groupMembers', () => {
  ...
  scenario('returns a single groupMember', async (scenario) => {
    const result = await groupMember({ cuid: scenario.groupMember.one.cuid })
  ...
  scenario('updates a groupMember', async (scenario) => {
    const original = await groupMember({ cuid: scenario.groupMember.one.cuid })
  ...
  scenario('deletes a groupMember', async (scenario) => {
    const original = await deleteGroupMember({
      cuid: scenario.groupMember.one.cuid,
    })

    const result = await groupMember({ cuid: original.cuid })
