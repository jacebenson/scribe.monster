...
let table = 'promptTrainingData'

export const createPromptTrainingData = async ({ input }) => {
  try {
...
export const promptTrainingDatas = async ({
  filter,
  skip,
  orderBy,
  q,
  ...
  try {
    ...
    let _take = (() => {
      let limit =
        take ||
        parseInt(preferences['prompttrainingdata.pageSize'], 10) ||
...
export const promptTrainingData = async ({ id }) => {
  try {
...
export const updatePromptTrainingData = async ({ id, input }) => {
  try {
...
export const deletePromptTrainingData = async ({ id }) => {
  try {
...
export const PromptTrainingData = {
  submittedBy: (_obj, { root }) =>
    db[table].findUnique({ where: { id: root.id } }).submittedBy(),
}
