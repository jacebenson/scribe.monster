...
let getFieldsAndSelect = async ({ table, formDefinition, listDefinition }) => {
  ...
  if(!definedFields) {
    throw new Error(`No fields defined for ${table}, update formFieldDefinitions.js`)
  }
...
export const readRecord = async ({ table, cuid }) => {
  ...
  console.log({ file: 'api/src/services/lists/lists.js', function: 'readRecord', table, cuid })
  let returnObj = {};
  returnObj.table = table;
  returnObj.cuid = cuid;
  returnObj.select = {cuid:true}
  let { fields, select, pascalTable } = await getFieldsAndSelect({ table, formDefinition: fieldDefinitions })
  returnObj.fields = fields
  returnObj.select = select

  let record = null
  // trigger before read rules
  if(cuid) { record = await db[pascalTable].findUnique({ where: { cuid }, select: returnObj.select, }) }
  // trigger after read rules
  returnObj.result = record
  return returnObj
}
// instead of list definitions... move this to a table...
export const readRecords = async ({ table, page, where, skip, orderBy, take }) => {
  try {
    ...
    if (where) {
      ...
      whereArray.forEach((item, index) => {
        ...
        if (whereArray.length % 3 !== 0) { throw new Error('Invalid where clause') }
        if(index % 3 === 0) {
          let key = item
    ...
    let { fields, select } = await getFieldsAndSelect({ table, listDefinition: listDefinitions })
    returnObj.fields = fields
    returnObj.select = select
    ...
    console.log({ file: 'api/src/services/lists/lists.js', function: 'readRecords', error })
