...
let table = 'memoryChunk'

export const createMemoryChunk = async ({ input }) => {
  try {
...
export const memoryChunks = async ({ filter, skip, orderBy, q, take }) => {
  try {
    let preferences = context.currentUser.preferences
    let _take = (() => {
      let limit =
        take ||
        parseInt(preferences['memorychunk.pageSize'], 10) ||
...
export const memoryChunk = async ({ cuid }) => {
  try {
...
export const updateMemoryChunk = async ({ cuid, input }) => {
  try {
...
export const deleteMemoryChunk = async ({ cuid }) => {
  try {
...
export const MemoryChunk = {
  memory: (_obj, { root }) =>
    db[table].findUnique({ where: { cuid: root.cuid } }).memory(),
}
