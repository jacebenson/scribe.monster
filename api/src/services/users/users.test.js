...
describe('users', () => {
  ...
  scenario('returns a single user', async (scenario) => {
    const result = await user({ cuid: scenario.user.one.cuid })
  ...
  scenario('updates a user', async (scenario) => {
    const original = await user({ cuid: scenario.user.one.cuid })
  ...
  scenario('deletes a user', async (scenario) => {
    const original = await deleteUser({ cuid: scenario.user.one.cuid })
    const result = await user({ cuid: original.cuid })
