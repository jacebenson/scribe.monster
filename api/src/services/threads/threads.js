...
export const threads = async ({ filter, skip, orderBy, q, take }) => {
  try {
    let preferences = context.currentUser.preferences
    let _take = (() => {
      let limit =
        take ||
        parseInt(preferences['thread.pageSize'], 10) ||
...
export const thread = async ({ cuid }) => {
  try {
...
export const updateThread = async ({ cuid, input }) => {
  try {
...
export const deleteThread = async ({ cuid }) => {
  try {
