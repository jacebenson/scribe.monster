...
export const memories = async ({ filter, skip, orderBy, q, take }) => {
  try {
    let preferences = context.currentUser.preferences
    let _take = (() => {
      let limit =
        take ||
        parseInt(preferences['memory.pageSize'], 10) ||
...
export const memory = async ({ cuid }) => {
  try {
...
export const updateMemory = async ({ cuid, input }) => {
  try {
...
export const deleteMemory = async ({ cuid }) => {
  try {
