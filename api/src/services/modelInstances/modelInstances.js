...
let table = 'modelInstance'

export const createModelInstance = async ({ input }) => {
  try {
...
export const modelInstances = async ({ filter, skip, orderBy, q, take }) => {
  try {
    let preferences = context.currentUser.preferences
    let _take = (() => {
      let limit =
        take ||
        parseInt(preferences['modelinstance.pageSize'], 10) ||
