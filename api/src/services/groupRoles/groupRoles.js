...
export const groupRoles = async ({ filter, skip, orderBy, q, take }) => {
  try {
    let _take = (() => {
      let limit =
        take ||
        parseInt(context.currentUser.preferences['groupRoles.pageSize'], 10) ||
...
export const groupRole = async ({ cuid }) => {
  try {
...
export const updateGroupRole = async ({ cuid, input }) => {
  try {
...
export const deleteGroupRole = async ({ cuid }) => {
  try {
