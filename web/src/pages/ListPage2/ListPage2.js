...
let parseParamsAndVariables = ({ params, page, take, orderBy, where }) => {
  ...
  if (params && typeof params === 'string') {
    ...
    if (page) paramsObject.page = page
    if (take) paramsObject.take = take
  ...
  if (
    ...
    if (params?.take) variables.take = params.take || take
    if (params?.page) {
      let skip = ((params?.page || page) - 1) * (params?.take || 10)
      variables.skip = skip
    }
    if (params?.orderBy) variables.orderBy = params.orderBy || orderBy
    if (params?.filter) variables.filter = params.filter
    if (params?.q) variables.q = params.q
    if (params?.where) variables.where = params.where || where
  }
...
const ListPage = ({
  ...
  let { paramsObject, variables } = parseParamsAndVariables({
    params,
    page: rawPage,
    take: rawTake,
    where: rawWhere,
  ...
  let { camelTable, pascalTable, pluralTable, spacedTable } = tableNames({
    table,
  })
  ...
  let [page, setPage] = useState(paramsObject?.page || rawPage || 1)
  let [take, setTake] = useState(paramsObject?.take || rawTake || 10)
  let [orderBy, setOrderBy] = useState(paramsObject?.orderBy || {})
  let [where, setWhere] = useState(paramsObject?.where || rawWhere || {})
  ...
  if (listState === 'loading') {
    return (
      <Box backgroundColor={'white'} borderRadius={'lg'} boxShadow={'lg'} p={4}>
        <Heading pb={2}>{spacedTable}</Heading>
                        ...
                        {sortable && !sortedBy && (
                          <IconButton
                            ...
                            onClick={() => {
                              ...
                              setTake(take)
                              setOrderBy({
                                [field.name]: sortedDirection || 'desc',
                              })
                              navigate(
                                '/list/' +
                                  camelTable +
                    ...
                    <Tr key={row.cuid}>
                      {schema &&
                        ...
                        schema.fields.map((field, fieldIndex) => {
                          ...
                          let referenceCuid =
                            row?.[field.name]?.[field.definition.value]
                          let genericFormLink = routes.formEdit({
                            table: camelTable,
                            cuid: row.cuid,
                          })
                          if (fieldIndex === 0) {
                          ...
                          if (reference && referenceCuid) {
                            let referenceTable = camelCase(field.name, {
                              pascalCase: false,
                            })
                            return (
                              <Td key={key}>
        ...
        <Center>
          <Box>
            ...
            <Center>
              <Text>
                {paramsObject?.page || 1} /{' '}
                {Math.ceil(count / (paramsObject?.take || 10))}
              </Text>
            </Center>
